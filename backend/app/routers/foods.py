...
def list_foods(
    ...
    return paginate(query.order_by(Food.created_at.desc()), page, page_size)

@router.get("/all", response_model=list[FoodOut])
def list_foods_all(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return db.query(Food).options(joinedload(Food.supplier)).order_by(Food.name).all()
...
def update_food(food_id: int, payload: FoodUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def delete_food(food_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    food = db.query(Food).filter(Food.id == food_id).first()
    if not food:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
